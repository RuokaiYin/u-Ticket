...
class QConv2d(nn.Conv2d):
    ...
    def forward(self, s_input):
        ...
        qweight = w_q(self.weight, self.num_bits_weight)
        output = F.conv2d(s_input, qweight, self.bias, self.stride,
...
class VGG(nn.Module):
    def __init__(
        self, features: nn.Module, num_classes: int = 10, total_timestep: int = 4
    ) -> None:
        super().__init__()
...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False) -> nn.Sequential:
    ...
    for v in cfg:
        if v == "M":
            ...
            v = cast(int, v)
            conv2d = QConv2d(in_channels, v, kernel_size=3, padding=1,num_bits_weight=4)
            if batch_norm:
