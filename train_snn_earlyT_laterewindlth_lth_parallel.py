...
def main():
    ...
    utils.checkdir(f"{os.getcwd()}/ETsnn_laterewind_lth_par/{args.arch}/{args.dataset}/round{args.round}")
    torch.save(model.state_dict(), f"{os.getcwd()}/ETsnn_laterewind_lth_par/{args.arch}/{args.dataset}/round{args.round}/initial_state_dict.pth.tar")
    ...
    for _ite in range(ITERATION):
        if not _ite == 0:
            ...
            check_mask  = copy.deepcopy(mask)

            layerwise_u_list_avg, _, _, dyn,was,lat  = utilization_operation_network(check_mask,check_model,n_PE=n_pe,operation="check",arch_style=arc_style)
            print("---> At pruning iteration: ", _ite)
            print("---> Original LTH-ET without recovering...")
            print("avg overall Layerwise Utilization: ", (sum(layerwise_u_list_avg) / len(layerwise_u_list_avg)))
            print("avg Layerwise Utilization: ", layerwise_u_list_avg)
        ...
        for iter_ in range(args.end_iter - rewinding_epoch):
            ...
            if (iter_+1) % args.valid_freq == 0:
                ...
                if accuracy > best_accuracy:
                    ...
                    utils.checkdir(f"{os.getcwd()}/ETsnn_laterewind_lth_par/{args.arch}/{args.dataset}/round{args.round}")
                    torch.save(model,
                               f"{os.getcwd()}/ETsnn_laterewind_lth_par/{args.arch}/{args.dataset}/round{args.round}/{_ite}_model.pth.tar")
            ...
            if _ite == 0 and iter_ == args.rewinding_epoch:
                ...
                torch.save(initial_state_dict,
                           f"{os.getcwd()}/ETsnn_laterewind_lth_par/{args.arch}/{args.dataset}/round{args.round}/initial_state_dict_rewind.pth.tar")
        ...
        tt2 = time.time()
        print(f'---> Training time for iteration {_ite}: {(tt2-tt1):.4f}s')
        bestacc[_ite]=best_accuracy

        # Dumping mask
        utils.checkdir(f"{os.getcwd()}/dumps/ETsnn_laterewind_lth_par/{args.arch}/{args.dataset}/round{args.round}")
        with open(f"{os.getcwd()}/dumps/ETsnn_laterewind_lth_par/{args.arch}/{args.dataset}/round{args.round}/mask_{comp1}.pkl",
                  'wb') as fp:
            pickle.dump(mask, fp)

        # Making variables into 0
        print(f'---> Best accuracy at pruning iteration {_ite}: {best_accuracy:.2f}%')
        best_accuracy = 0
