...
def main():
    ...
    for round_ in range(ITERATION):
        ...
        if not round_ == 0:
            ...
            utils.checkdir(f"{os.getcwd()}/dumps/snn_laterewind_lth/{args.arch}/{args.dataset}/round{args.round}/{arc_style}/{n_pe}")
            with open(
                    f"{os.getcwd()}/dumps/snn_laterewind_lth/{args.arch}/{args.dataset}/round{args.round}/{arc_style}/{n_pe}/mask_{comp1}.pkl",
                    'wb') as fp:
                pickle.dump(mask, fp)
        ...
        for iter_ in range(args.end_iter - rewinding_epoch):
            ...
            if (iter_+1) % args.valid_freq == 0:
                ...
                if accuracy > best_accuracy:
                    ...
                    utils.checkdir(f"{os.getcwd()}/snn_laterewind_lth/{args.arch}/{args.dataset}/round{args.round}/{arc_style}/{n_pe}")
                    torch.save(model,
                               f"{os.getcwd()}/snn_laterewind_lth/{args.arch}/{args.dataset}/round{args.round}/{arc_style}/{n_pe}/{round_}_model.pth.tar")
