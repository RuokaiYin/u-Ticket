...
def main():
    ...
    utils.checkdir(f"{os.getcwd()}/ETsnn_laterewind_ut_par/{args.arch}/{args.dataset}/round{args.round}")
    torch.save(model.state_dict(), f"{os.getcwd()}/ETsnn_laterewind_ut_par/{args.arch}/{args.dataset}/round{args.round}/initial_state_dict.pth.tar")
    ...
    for _ite in range(ITERATION):
        ...
        for iter_ in range(args.end_iter - rewinding_epoch):
            ...
            if (iter_+1) % args.valid_freq == 0:
                ...
                if accuracy > best_accuracy:
                    ...
                    utils.checkdir(f"{os.getcwd()}/ETsnn_laterewind_ut_par/{args.arch}/{args.dataset}/round{args.round}")
                    torch.save(model,
                               f"{os.getcwd()}/ETsnn_laterewind_ut_par/{args.arch}/{args.dataset}/round{args.round}/{_ite}_model.pth.tar")
            ...
            if _ite == 0 and iter_ == args.rewinding_epoch:
                ...
                torch.save(initial_state_dict,
                           f"{os.getcwd()}/ETsnn_laterewind_ut_par/{args.arch}/{args.dataset}/round{args.round}/initial_state_dict_rewind.pth.tar")
        ...
        # Dumping mask
        utils.checkdir(f"{os.getcwd()}/dumps/ETsnn_laterewind_ut_par/{args.arch}/{args.dataset}/round{args.round}")
        with open(f"{os.getcwd()}/dumps/ETsnn_laterewind_ut_par/{args.arch}/{args.dataset}/round{args.round}/mask_{comp1}.pkl",
                  'wb') as fp:
            pickle.dump(mask, fp)
